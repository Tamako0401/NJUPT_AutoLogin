...
| `-r` | restart_device 网路设备  | -      | 当出现登录失败且返回空时可尝试开启 [#16](https://github.com/s235784/NJUPT_AutoLogin/issues/16) |
| `-T` | sleep_time 登录等待时间    | `1`    | 重启网络设备后等待指定时间后再尝试登录，与`-r`配合使用                                                 |
...
> [!TIP]
> 更详细的教程请移步 [Nosora 的博客](https://nosora.dev/archives/204#header-id-4)。
...
如果需要在路由器上长期运行，尤其是单线多拨或多账号场景，不建议把账号和密码直接写进 LuCI 计划任务，~~否则你将每几分钟就看到账号密码出现在日志里，非常不安全~~。推荐把账号配置放在路由器本地的独立文件中，并限制权限：
